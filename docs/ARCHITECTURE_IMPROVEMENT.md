...
### 第一阶段：后端基础重构 (Backend Foundation)
1.  初始化 Express + TypeScript 项目结构。
2.  迁移 `server.js` 中的认证和文件操作逻辑到新的 Controller/Service 结构。
3.  前端适配新的 API 接口。

### 第二阶段：状态管理与 AI 代理 (State & AI Proxy)
1.  引入 Zustand，逐步替换 `useWorldModel`。
2.  在后端创建 AI Service，将前端的 `geminiService.ts` 逻辑迁移至后端。
3.  实现 Prompt 模板化。

### 第三阶段：性能与体验 (Performance & UX)
1.  实现结构化 JSON 存储与 Git 深度集成。
2.  实现 AI 生成的流式响应。
3.  前端列表虚拟化优化。
