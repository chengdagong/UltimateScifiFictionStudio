    ...
    for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
        try {
            if (config.provider === 'openai') {
                ...
                const model = ai.models.getGenerativeModel ? ai.models.getGenerativeModel(modelParams) : ai.models.getModel ? ai.models.getModel(modelParams) : (ai as any).getGenerativeModel(modelParams);

                // Convert history to Google format
                // Note: Google SDK history format is { role: 'user' | 'model', parts: [{ text: string }] }
