...
const TechTreeView: React.FC<TechTreeViewProps> = ({
  ...
  useEffect(() => {
    ...
    try {
        ...
        const leftControl = node.append("g")
            ...
            .on("click", (e: any, d: any) => {
                e.stopPropagation();
                // Open Context Menu instead of direct action
                setActionMenu({
                    x: e.pageX,
                    y: e.pageY,
                    nodeId: d.id,
  ...
  const handleMenuOptionClick = (type: 'ai' | 'manual') => {
      ...
      const { nodeId, direction } = actionMenu;
  ...
  return (
    <div className="flex h-full bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden relative">
        ...
        <div className="flex-1 flex flex-col relative">
           ...
           {actionMenu && (
              <div 
                 className="absolute z-20 bg-white rounded-lg shadow-xl border border-slate-200 p-1 flex flex-col min-w-[150px] animate-fadeIn"
                 style={{ left: Math.min(actionMenu.x - 250, 800), top: Math.min(actionMenu.y - 80, 600) }} 
              >
                  <div className="px-2 py-1 text-[10px] font-bold text-slate-400 uppercase tracking-wider border-b border-slate-100 mb-1">
                      {actionMenu.direction === 'dependency' ? '添加前置技术' : '添加后续技术'}
                  </div>
                  <button 
                  ...
                  <button 
                     ...
                     className="flex items-center gap-2 px-3 py-2 text-sm text-slate-700 hover:bg-indigo-50 hover:text-indigo-600 rounded text-left"
                  >
                     <PenTool className="w-4 h-4" /> 手动创建
                  </button>
              </div>
           )}
           ...
           <div ref={containerRef} className="flex-1 bg-slate-50/50 cursor-grab active:cursor-grabbing" onClick={() => setSelectedNodeId(null)}>
