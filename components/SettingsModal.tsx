    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm">
      <div className="bg-white w-full h-full md:h-auto md:w-auto md:rounded-xl md:shadow-2xl md:max-w-md overflow-hidden flex flex-col animate-fadeIn">
        ...
        <div className="p-6 space-y-6 flex-1 overflow-y-auto">
          ...
          <div className="space-y-4">
            <label className="text-xs font-bold text-slate-500 uppercase flex items-center gap-1.5 border-b border-slate-100 pb-1">
              <Server className="w-3.5 h-3.5" /> {t('settings_api_config')}
            </label>
            ...
            <div className="space-y-2">
              <label className="text-xs font-bold text-slate-500 uppercase flex items-center gap-1.5">
                <Globe className="w-3.5 h-3.5" /> {t('settings_base_url')}
              </label>
              <input
                type="text"
                value={formData.baseUrl}
                onChange={e => handleChange('baseUrl', e.target.value)}
                placeholder={formData.provider === 'google' ? t('settings_placeholder_url_google') : t('settings_placeholder_url_openai')}
              ...
              <p className="text-[10px] text-slate-500 bg-slate-50 p-2 rounded border border-slate-100">
                {formData.provider === 'google'
                  ? t('settings_help_url_google')
                  : t('settings_help_url_openai')}
              </p>
            </div>
            ...
            <div className="space-y-2">
              <label className="text-xs font-bold text-slate-500 uppercase flex items-center gap-1.5">
                <Key className="w-3.5 h-3.5" /> {t('settings_api_key')}
              </label>
              <input
                type="password"
                value={formData.apiKey}
                onChange={e => handleChange('apiKey', e.target.value)}
                placeholder={t('settings_placeholder_api_key', { provider: formData.provider === 'google' ? 'Google' : 'Provider' })}
                className="w-full px-3 py-3 md:py-2 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 font-mono text-sm"
              />
            </div>
            ...
            <div className="space-y-2">
              <label className="text-xs font-bold text-slate-500 uppercase flex items-center gap-1.5">
                <Box className="w-3.5 h-3.5" /> {t('settings_model_name')}
              </label>
              <input
                type="text"
        ...
        <div className="px-6 py-4 bg-slate-50 border-t border-slate-100 flex justify-between items-center shrink-0 safe-area-pb">
          ...
          >
            <RotateCcw className="w-3 h-3" /> {t('action_reset_default')}
          </button>
          <button
            onClick={handleSave}
          ...
          >
            <Save className="w-4 h-4" /> {t('action_save_config')}
          </button>
        </div>
      </div>
    </div>
