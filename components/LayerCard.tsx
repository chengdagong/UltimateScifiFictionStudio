...
const CategoryIcon: React.FC<{ category: EntityCategory, className?: string }> = ({ category, className = "w-4 h-4" }) => {
  ...
  const getIconData = () => {
    switch (category) {
      case EntityCategory.PERSON: return { Icon: User, title: t('cat_person') };
      case EntityCategory.ORGANIZATION: return { Icon: Building2, title: t('cat_org') };
      case EntityCategory.TECHNOLOGY: return { Icon: Cpu, title: t('cat_tech') };
      case EntityCategory.RESOURCE: return { Icon: Coins, title: t('cat_resource') };
      case EntityCategory.BELIEF: return { Icon: Scale, title: t('cat_belief') };
      case EntityCategory.PLACE: return { Icon: MapPin, title: t('cat_place') };
      case EntityCategory.EVENT: return { Icon: Calendar, title: t('cat_event') };
      default: return { Icon: HelpCircle, title: t('cat_unknown') };
    }
  };
...
const CategoryLabel: React.FC<{ category: EntityCategory }> = ({ category }) => {
  ...
  const labels: Record<string, string> = {
    [EntityCategory.PERSON]: t('cat_label_person'),
    [EntityCategory.ORGANIZATION]: t('cat_label_org'),
    [EntityCategory.TECHNOLOGY]: t('cat_label_tech'),
    [EntityCategory.RESOURCE]: t('cat_label_resource'),
    [EntityCategory.BELIEF]: t('cat_label_belief'),
    [EntityCategory.PLACE]: t('cat_label_place'),
    [EntityCategory.EVENT]: t('cat_label_event'),
    [EntityCategory.UNKNOWN]: t('cat_label_unknown') || t('cat_unknown')
  };
        ...
        {isAdding && (
          <div className="mb-4 p-4 bg-gray-50 rounded-lg border border-gray-200 animate-fadeIn">
            <div className="flex gap-2 mb-2">
              <input
                type="text"
                placeholder={layerDef.isTimeDimension ? t('layer_placeholder_name_event') : t('layer_placeholder_name_entity')}
            ...
            <textarea
              placeholder={t('layer_placeholder_desc')}
            ...
            />
            <div className="flex justify-end gap-2">
              <button onClick={() => setIsAdding(false)} className="text-sm text-gray-500 px-3 py-1">{t('action_cancel')}</button>
              <button onClick={handleAdd} className="text-sm bg-gray-800 text-white px-3 py-1 rounded hover:bg-black">{t('action_save')}</button>
            </div>
          </div>
        )}
